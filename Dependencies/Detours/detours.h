...
#define DetourStart(Original, Hook)                                  \
do                                                                     \
{                                                                      \
    if (DetourTransactionBegin() == NO_ERROR)                          \
    {                                                                  \
        DetourUpdateThread(GetCurrentThread());                        \
        if (DetourAttach(reinterpret_cast<void**>(&(Original)), reinterpret_cast<void*>(Hook)) == NO_ERROR) \
...
#define DetourRemove(Original, Hook)                                 \
do                                                                    \
{                                                                     \
    if (DetourTransactionBegin() == NO_ERROR)                         \
    {                                                                 \
        DetourUpdateThread(GetCurrentThread());                       \
        if (DetourDetach(reinterpret_cast<void**>(&(Original)), reinterpret_cast<void*>(Hook)) == NO_ERROR) \
            DetourTransactionCommit();                                \
        else                                                          \
            DetourTransactionAbort();                                 \
    }                                                                 \
